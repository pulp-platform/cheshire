...
- Xilinx VCU108 with Vivado `>= 2020.2`
...
### Xilinx VCU108 (`vcu108`), VCU118 (`vcu118`) and VCU128 (`vcu128`)

Since there are no (or not enough) switches on these boards, the boot mode must be selected using Virtual IOs (see [Virtual IOs](#virtual_ios) below).
