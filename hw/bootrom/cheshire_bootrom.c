...
int boot_spi_sdcard(uint64_t core_freq, uint64_t rtc_freq) {
    ...
    spi_sdcard_t device = {
        .spi_freq = 25*1000*1000,  // 20MHz (Maximum is 25MHz)
        .csid = 0,
...
int boot_i2c_24xx1025(uint64_t core_freq) {
    // Initialize device handle
    dif_i2c_t i2c;
    CHECK_CALL(i2c_24xx1025_init(&i2c, core_freq))
...
int main() {
    ...
    case 3:
        return boot_i2c_24xx1025(core_freq);
    default:
        return boot_passive(core_freq);
    }
}
