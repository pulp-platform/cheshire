...
module tb_cheshire_soc #(
  ...
  string        boot_hex;
  logic [1:0]   boot_mode;
  logic [2:0]   preload_mode; // 0: JTAG, 1: Serial Link, 2: UART, 3: MEM, 4: haltreq debug entry, 5: WFI wake-up
  bit   [31:0]  exit_code;
  bit           openocd_mode;
    ...
    if (boot_mode == 0) begin
      ...
      case (preload_mode)
        0: begin      // JTAG
          ...
          fix.vip.jtag_wait_for_eoc(exit_code);
        end 5: begin  // JTAG haltreq wakes hart waiting in WFI
          fix.vip.jtag_init();
          fix.vip.jtag_wfi_wakeup_test();
          fix.vip.jtag_elf_run(preload_elf);
          fix.vip.jtag_wait_for_eoc(exit_code);
        end
        default: begin
