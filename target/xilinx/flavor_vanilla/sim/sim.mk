...
chs-ip-sim-scripts := $(addsuffix /questa/compile.do, $(addprefix $(CHS_XIL_SIM_DIR)/ips/, $(chs_xilinx_ips_names_vanilla)))

# Getting the DDR model requires exporting the Vivado example project for the controller's IP
chs-ddr-example-name := $(filter xlnx_mig_%,$(chs_xilinx_ips_names_vanilla))_ex
chs-ddr-sim-script := $(CHS_XIL_SIM_DIR)/ips/$(chs-ddr-example-name)/questa/compile.do

chs-vivado-env-sim := $(chs_vivado_env_vanilla) \
...
$(CHS_XIL_SIM_DIR)/add_sources_vsim.tcl:
	$(BENDER) script vsim -t sim -t test $(chs_xilinx_targs_vanilla) --vlog-arg="$(chs-xil-vlog-args)" > $@
...
chs-xil-sim: $(CHS_XIL_DIR)/flavor_vanilla/cheshire.xpr $(XILINX_SIMLIB_PATH)/modelsim.ini $(chs-ddr-sim-script) $(chs-ip-sim-scripts) $(CHS_XIL_SIM_DIR)/add_sources_vsim.tcl
	...
	cd $(CHS_XIL_SIM_DIR) && IPS="$(chs_xilinx_ips_names_vanilla)" questa-2022.3 vsim -work work -do "run_simulation.tcl"

testt:
	echo $(filter xlnx_mig_%,$(chs_xilinx_ips_names_vanilla))_ex
