...
# `set_max_delay -datapath only` implicitly waives all undesired checks between clocks (setup with
...
set_input_delay -min -clock $usb_clk [expr { 0.10 * $USB_TCK }] [get_ports {usb_d*}]
set_input_delay -max -clock $usb_clk [expr { 0.35 * $USB_TCK }] [get_ports {usb_d*}]

set_output_delay -min -clock $usb_clk [expr { 0.10 * $USB_TCK }] [get_ports {usb_d*}]
set_output_delay -max -clock $usb_clk [expr { 0.35 * $USB_TCK }] [get_ports {usb_d*}]

###############
# Serial Link #
###############

# Route these clocks without any special requirements, do not buffer input clock (very few sinks)
# We try to define this first as doing it after clock definition does not work??
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets -of [get_ports {slink_rcv_clk*}]]
set_property CLOCK_BUFFER_TYPE NONE [get_ports {slink_rcv_clk_i[0]}]

# Assume half the speed of system clock for now
set SLINK_TCK [expr $SOC_TCK * 2]
create_clock -period $SLINK_TCK -name slink_in_clk [get_ports slink_rcv_clk_i[0]]

# Create generated clock
set SLINK_OUT_CPIN [get_pins \
    [all_fanin -quiet -flat -startpoints_only [get_ports slink_rcv_clk_o[0]]]]
create_generated_clock -name slink_out_clk -divide_by 2 -source $soc_clk_pin $SLINK_OUT_CPIN

# Secure incoming CDC
set_max_delay -datapath_only -from slink_in_clk -to $soc_clk [expr { 0.67 * $SOC_TCK }]
set_max_delay -datapath_only -from $soc_clk -to slink_in_clk [expr { 0.67 * $SOC_TCK }]

# Assume long region of instability on input data.
# "Long" is with respect to a half-period here as we sample a DDR signal
set_input_delay -min -clock slink_in_clk [expr { 0.10 * $SLINK_TCK }] [get_ports {slink_i*}]
set_input_delay -max -clock slink_in_clk [expr { 0.40 * $SLINK_TCK }] [get_ports {slink_i*}]

# Ensure fast-propagating and aligned outputs
set_output_delay -min -clock slink_out_clk [expr { 0.10 * $SLINK_TCK }] [get_ports {slink_o*}]
set_output_delay -max -clock slink_out_clk [expr { 0.15 * $SLINK_TCK }] [get_ports {slink_o*}]
set_output_delay -min -clock slink_out_clk [expr { 0.10 * $SOC_TCK }] [get_ports {slink_rcv_clk_o*}]
set_output_delay -max -clock slink_out_clk [expr { 0.15 * $SOC_TCK }] [get_ports {slink_rcv_clk_o*}]
...
## PMOD Header JC (SLINK 4x RX)
set_property -dict { PACKAGE_PIN AC26  IOSTANDARD LVCMOS33 } [get_ports { slink_i[0][0] }]; #IO_L19P_T3_13 Sch=jc[1]
set_property -dict { PACKAGE_PIN AJ27  IOSTANDARD LVCMOS33 } [get_ports { slink_i[0][1] }]; #IO_L20P_T3_13 Sch=jc[2]
set_property -dict { PACKAGE_PIN AH30  IOSTANDARD LVCMOS33 } [get_ports { slink_i[0][2] }]; #IO_L18N_T2_13 Sch=jc[3]
set_property -dict { PACKAGE_PIN AK29  IOSTANDARD LVCMOS33 } [get_ports { slink_i[0][3] }]; #IO_L15P_T2_DQS_13 Sch=jc[4]
...
set_property -dict { PACKAGE_PIN AK28  IOSTANDARD LVCMOS33 } [get_ports { slink_rcv_clk_i[0] }]; #IO_L20N_T3_13 Sch=jc[10]

## PMOD Header JD (SLINK 4x TX)
set_property -dict { PACKAGE_PIN V27   IOSTANDARD LVCMOS33 } [get_ports { slink_o[0][0] }]; #IO_L16N_T2_A15_D31_14 Sch=jd[1]
set_property -dict { PACKAGE_PIN Y30   IOSTANDARD LVCMOS33 } [get_ports { slink_o[0][1] }]; #IO_L8P_T1_13 Sch=jd[2]
set_property -dict { PACKAGE_PIN V24   IOSTANDARD LVCMOS33 } [get_ports { slink_o[0][2] }]; #IO_L23N_T3_A02_D18_14 Sch=jd[3]
set_property -dict { PACKAGE_PIN W22   IOSTANDARD LVCMOS33 } [get_ports { slink_o[0][3] }]; #IO_L24N_T3_A00_D16_14 Sch=jd[4]
...
set_property -dict { PACKAGE_PIN W21   IOSTANDARD LVCMOS33 } [get_ports { slink_rcv_clk_o[0] }]; #IO_L24P_T3_A01_D17_14 Sch=jd[10]
