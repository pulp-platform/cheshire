...
static inline uint16_t __spi_sdcard_crc16(uint8_t *data, uint64_t len) {
    uint16_t state = 0;
    for (uint64_t i = 0; i < len; ++i)
        state = (__spi_sdcard_crc16_lookup[(data[i] ^ (state >> 8)) & 0xFFU] ^ (state << 8)) & 0xFFFFU;
...
static inline int __spi_sdcard_xfer_csaat(spi_sdcard_t *handle, void *rx, void *tx, uint64_t len) {
    ...
    // Receive bytes
    if (tx)
        seg = (dif_spi_host_segment_t){kDifSpiHostSegmentTypeTx, {.tx = {
            .width = kDifSpiHostWidthStandard,
            .buf = tx,
            .length = len }}};
    // Send bytes
    else if (rx)
        seg = (dif_spi_host_segment_t){kDifSpiHostSegmentTypeRx, {.rx = {
            .width = kDifSpiHostWidthStandard,
            .buf = rx,
            .length = len }}};
    // Issue dummy cycles (here, len is *cycles* and not bytes)
    else
        seg = (dif_spi_host_segment_t){kDifSpiHostSegmentTypeDummy, {.dummy = {
            .width = kDifSpiHostWidthStandard,
            .length = 8*len }}};
...
static inline int __spi_sdcard_csfree(spi_sdcard_t *handle) {
    dif_spi_host_segment_t seg = {kDifSpiHostSegmentTypeDummy, {.dummy = {
        .width = kDifSpiHostWidthStandard,
        .length = 8 }}};
...
static inline int __spi_sdcard_cmd(spi_sdcard_t *handle, uint64_t cmd,
                                   spi_sdcard_resp_t resp, uint8_t *rdata,
                                   int csaat) {
    uint8_t rdummy;
    ...
    if ((cmd & 0xFF) == 0x4C)
        CHECK_CALL(__spi_sdcard_xfer_csaat(handle, &rdummy, NULL, 1))
    ...
    if (!csaat)
         CHECK_CALL(__spi_sdcard_csfree(handle));
    // Nothing went wrong
    return 0;
}

// Handle a command and check its response against an expected value or discard it
static inline int __spi_sdcard_cmd_check(spi_sdcard_t *handle, uint64_t cmd,
                                 spi_sdcard_resp_t resp, int check_exp, uint64_t exp) {
    uint64_t buf;
    CHECK_CALL(__spi_sdcard_cmd(handle, cmd, resp, (uint8_t*)&buf, 0))
    uint64_t resp_mask = (1 << (8UL*__spi_sdcard_resp_len[resp])) - 1;
    if (check_exp && ((buf & resp_mask) != (exp & resp_mask)))
        return buf & resp_mask;
    return 0;
}
...
static int __spi_sdcard_activate(spi_sdcard_t *handle) {
    ...
    /*CMD0*/ CHECK_CALL(__spi_sdcard_cmd_check(handle, 0x950000000040UL, kSpiSdcardRespR1, 1, 0x01UL))
    /*CMD8*/ CHECK_CALL(__spi_sdcard_cmd_check(handle, 0x87AA01000048UL, kSpiSdcardRespR7, 1, 0xAA01000001UL))
    uint8_t ret = 1;
    do {
        /*CMD55*/ CHECK_CALL(__spi_sdcard_cmd_check(handle, 0x650000000077UL, kSpiSdcardRespR1, 0, 0))
    ...
    /*CMD16*/ CHECK_CALL(__spi_sdcard_cmd_check(handle, 0x150002000050UL, kSpiSdcardRespR1, 1, 0x00UL))
    // Nothing went wrong
    return 0;
}
...
int __spi_sdcard_read_blocks(spi_sdcard_t *handle, void *buf, uint64_t block, uint64_t len,
    ...
    for (uint64_t b = 0; b < len; ++b) {
        ...
        if (check_crc && crc != __spi_sdcard_crc16((uint8_t*)block_buf, 512))
            return 0x21;
        // If this block is buffered, copy only relevant contents to destination
        if (first_block && last_block)
            __builtin_memcpy(block_dst + first_offs, block_swap + first_offs, last_len - first_offs);
        else if (first_block)
...
int __spi_sdcard_read(void *priv, void* buf, uint64_t addr, uint64_t len, int check_crc) {
    ...
    // Read blocks
    return __spi_sdcard_read_blocks((spi_sdcard_t*)priv, buf, block, len_blocks, swap,
                                    first_offs, last_len, check_crc);
}
