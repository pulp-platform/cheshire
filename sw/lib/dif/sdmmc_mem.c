...
sdmmc_mem_enable(struct sdmmc_softc *sc)
{
 ...
 mmc_mode:
	...
	if (ISSET(sc->sc_flags, SMF_SD_MODE)) {
		DPRINTF(("%s: detected SD Card\n",
			DEVNAME(sc)));
	} else {
		DPRINTF(("%s: detected MMC Card\n",
			DEVNAME(sc)));
	}
...
{
	DFUNC(sdmmc_mem_signal_voltage);
	int error;

	/*
	 * Stop the clock
	 */
	error = sdhc_bus_clock(sc->sch, 0, SDMMC_TIMING_LEGACY);
	if (error)
		return error;
	...
	error = sdhc_signal_voltage(sc->sch, signal_voltage);
	if (error)
		return error;
...
sdmmc_mem_scan(struct sdmmc_softc *sc)
{
	...
	sc->sc_card.sc = sc;
	sc->sc_card.cur_blklen = sdhc_host_maxblklen(sc->sch);
	sc->sc_card.rca = 1;
	 ...
	 */
	if ((error = sdmmc_set_relative_addr(sc, &sc->sc_card)) != 0) {
		DPRINTF(("%s: can't set mem RCA\n", DEVNAME(sc)));
		return error;
	}
	...
	cmd.c_arg = MMC_ARG_RCA(sc->sc_card.rca);
	cmd.c_flags = SCF_CMD_AC | SCF_RSP_R2;

	if ((error = sdmmc_mmc_command(sc, &cmd)) != 0) {
		return 1;
	}

	if ((error = sdmmc_decode_csd(sc, cmd.c_resp, &sc->sc_card)) != 0) {
		return error;
	}
...
sdmmc_mem_set_bus_width(struct sdmmc_function *sf, int width)
{
	...
	if (error == 0)
		error = sdhc_bus_width(sc->sch, width);
	return error;
}
...
sdmmc_be512_to_bitfield512(sdmmc_bitfield512_t *buf) {
	DFUNC(sdmmc_be512_to_bitfield512);
...
{
	DFUNC(sdmmc_mem_select_transfer_mode);
...
{
	DFUNC(sdmmc_mem_execute_tuning);
	...
	// TODO
	//return sdhc_execute_tuning(sc->sch, timing);
	return 0;
}
...
sdmmc_mem_sd_init(struct sdmmc_softc *sc, struct sdmmc_function *sf)
{
	...
	error = sdhc_bus_clock(sc->sch, SDMMC_SDCLK_25MHZ,
	    SDMMC_TIMING_LEGACY);
	if (error) {
		DPRINTF(("%s: can't change bus clock\n", DEVNAME(sc)));
		return error;
	}
	...
	if (sf->scr.sd_spec >= SCR_SD_SPEC_VER_1_10 &&
		...
		if (error) {
			DPRINTF(("%s: switch func mode 0 failed\n", DEVNAME(sc)));
			return error;
		}
		...
		if (!ISSET(sc->sc_flags, SMF_UHS_MODE) &&
			...
			if (error) {
				DPRINTF(("%s: failed to recover UHS card\n", DEVNAME(sc)));
				return error;
			}
	...
	if (best_func != 0) {
		...
		if (error) {
			DPRINTF(("%s: switch func mode 1 failed:"
		...
		// delay(25);
		// TODO we can only wait for 1ms with timer
		sdmmc_delay(1000);
		...
		if (error) {
			DPRINTF(("%s: can't execute SD tuning\n", DEVNAME(sc)));
			return error;
		}
	}
...
{
	DFUNC(sdmmc_mem_mmc_init);

	int width, value;
	int card_type;
	int error = 0;
	u_int8_t* ext_csd = sc->scratch_buffer;
	int speed = 20000;
	int timing = SDMMC_TIMING_LEGACY;
	u_int32_t sectors = 0;

	error = sdhc_bus_clock(sc->sch, speed, timing);
	if (error) {
		DPRINTF(("%s: can't change bus clock\n", DEVNAME(sc)));
		return error;
	}
	...
	if (sf->csd.mmcver >= MMC_CSD_MMCVER_4_0) {
		...
		error = sdmmc_mem_send_cxd_data(sc,
		    MMC_SEND_EXT_CSD, ext_csd, 512);
		if (error != 0) {
			DPRINTF(("%s: can't read EXT_CSD\n", DEVNAME(sc)));
			return error;
		}
		...
		if (timing != SDMMC_TIMING_LEGACY) {
			...
			if (error != 0) {
				DPRINTF(("%s: can't re-read EXT_CSD\n", DEVNAME(sc)));
				return error;
			}
			if (ext_csd[EXT_CSD_HS_TIMING] != value) {
				DPRINTF(("%s, HS_TIMING set failed\n", DEVNAME(sc)));
				return EINVAL;
			}
		}

		error = sdhc_bus_clock(sc->sch, speed, SDMMC_TIMING_HIGHSPEED);
		if (error != 0) {
			DPRINTF(("%s: can't change bus clock\n", DEVNAME(sc)));
			return error;
		}
		...
		if (timing == SDMMC_TIMING_MMC_DDR52) {
			...
			error = sdhc_signal_voltage(sc->sch,
			    SDMMC_SIGNAL_VOLTAGE_180);
			if (error) {
				DPRINTF(("%s: can't switch signalling voltage\n",
				    DEVNAME(sc)));
				return error;
			}
		...
		if (timing == SDMMC_TIMING_MMC_HS200) {
			...
			error = sdmmc_mem_execute_tuning(sc, sf);
			if (error) {
				DPRINTF(("%s: can't execute MMC tuning\n", DEVNAME(sc)));
				return error;
			}
		}
	}
...
int
sdmmc_mem_read_block_subr(struct sdmmc_function *sf, int blkno, u_char *data, size_t datalen)
{
	DFUNC(sdmmc_mem_read_block_subr);
...
sdmmc_mem_single_read_block(struct sdmmc_function *sf, int blkno, u_char *data,
    size_t datalen)
{
	DFUNC(sdmmc_mem_single_read_block);
...
int
sdmmc_mem_write_block_subr(struct sdmmc_function *sf, int blkno, u_char *data, size_t datalen)
{
	DFUNC(sdmmc_mem_write_block_subr);
...
sdmmc_mem_single_write_block(struct sdmmc_function *sf, int blkno, u_char *data,
    size_t datalen)
{
	DFUNC(sdmmc_mem_single_write_block);
    ...
    size_t datalen)
{
	...
	if (!ISSET(sc->sc_caps, SMC_CAPS_DMA)) {
		error = sdmmc_mem_write_block_subr(sf, blkno,
		    data, datalen);
		goto out;
	}
