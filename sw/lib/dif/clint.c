...
uint64_t clint_get_mtime() {
    return (((uint64_t)*reg32(&__chs_clint_base_addr__, CLINT_MTIME_HIGH_REG_OFFSET)) << 32) |
           ((uint64_t)*reg32(&__chs_clint_base_addr__, CLINT_MTIME_LOW_REG_OFFSET));
}
...
void clint_set_mtimecmpx(uint64_t timer_idx, uint64_t value) {
    ...
    *reg32(&__chs_clint_base_addr__, CLINT_MTIMECMP_HIGH0_REG_OFFSET + mtimecmp_offs) = vhi;
    *reg32(&__chs_clint_base_addr__, CLINT_MTIMECMP_LOW0_REG_OFFSET + mtimecmp_offs) = vlo;
}
