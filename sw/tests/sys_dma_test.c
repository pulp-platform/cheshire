...
//    );
    ...
    PRINTF("1D sys DMA blocking memcpy from LLC-SPM to DDR3 on genesys2\r\n");
    // write from spm to dram (blocking)
    sys_dma_blk_memcpy(dram_free, spm_free, size_bytes);
    fence();

    PRINTF("1D sys DMA blocking memcpy from DDR3 to LLC-SPM on genesys2\r\n");
    // write from dram to spm (blocking)
    sys_dma_blk_memcpy(spm_free, dram_free, size_bytes);
    fence();
