...
#define LLC_RW_TEST_REG(NAME, VAL) \
    err = probe_rw(&__llc_base_addr__, NAME, VAL); \
    if (err) { \
...
int main(void) {
    ...
    uint32_t low = *reg32(&__llc_base_addr__, AXI_LLC_VERSION_LOW_REG_OFFSET);
    uint32_t high = *reg32(&__llc_base_addr__, AXI_LLC_VERSION_HIGH_REG_OFFSET);
