...
void __attribute__((naked)) chs_hmr_store_state() {
            ...
            HMR_CORE_OFFS) " \n\t"
                           "sll t2, t0, " QU(
                               HMR_CORE_SLL) " \n\t"
                                             "add t2, t2, t1 \n\t"
                                             "sw sp, " QU(
                                                 HMR_CORE_REGS_SP_STORE_REG_OFFSET) "(t2) \n\t"
        :
        :
        : "memory");
    ...
    __asm__ __volatile__("la t1, " QU(BaseHmr + HMR_DMR_CHECKPOINT) "\n\t"
                                                                    "sw ra, 0(t1) \n\t"
                         :
                         :
                         : "memory");
    ...
    // Re-enable caches
    __asm__ __volatile__("csrrwi x0, 0x7C1, 0x1 \n\t" : : : "memory");
            ...
            HMR_CORE_OFFS) " \n\t"
                           "sll t0, t0, " QU(
                               HMR_CORE_SLL) " \n\t"
                                             "add t0, t0, t1 \n\t"
                                             "lw sp, " QU(
                                                 HMR_CORE_REGS_SP_STORE_REG_OFFSET) "(t0) \n\t"
                                                                                    "slli sp, sp, "
                                                                                    "0x20 \n\t"
                                                                                    "srli sp, sp, "
                                                                                    "0x20 \n\t"
