...
inline uint32_t mmio_region_read32(mmio_region_t base, ptrdiff_t offset) {
  ...
  // Enforce proper word-size accesses on CVA6 (no other way)
  volatile register uint32_t ret;
  asm volatile (
    "add %[b],%[b],%[o];"
    "lwu %[r], 0(%[b]);"
    : [r]"=r"(ret), [b]"+r"(base.base)
    : [o]"r"(offset)
    : "memory"
  );
  return ret;
...
inline void mmio_region_write32(mmio_region_t base, ptrdiff_t offset,
  ...
  // Enforce proper word-size accesses on CVA6 (no other way)
  // Offset too big to be RISC-V reg offset
  asm volatile (
    "add %[b],%[b],%[o];"
    "sw %[v], 0(%[b]);"
    : [b]"+r"(base.base)
    : [v]"r"(value), [o]"r"(offset)
    : "memory"
  );
}
